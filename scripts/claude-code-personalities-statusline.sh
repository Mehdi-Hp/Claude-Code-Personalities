...
VERSION="0.0.3"  # Default fallback
...
if [[ ! -f "$UPDATE_CHECK_FILE" ]]; then
  ...
  if latest=$(curl -sL --max-time 1 https://api.github.com/repos/kumamaki/claude-code-personalities/releases/latest 2>/dev/null | jq -r ".tag_name" 2>/dev/null); then
