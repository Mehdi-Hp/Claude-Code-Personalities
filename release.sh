...
See [CLAUDE.md](https://github.com/kumamaki/claude-code-personalities/blob/main/CLAUDE.md) for:
...
if ! command -v gh &> /dev/null; then
    ...
    echo -e "${CYAN}   https://github.com/kumamaki/claude-code-personalities/releases/new${NC}"
else
...
while [[ $RETRY_COUNT -lt $MAX_RETRIES ]] && [[ -z "$SHA256" ]]; then
    SHA256=$(curl -sL "https://github.com/kumamaki/claude-code-personalities/archive/$TAG.tar.gz" | shasum -a 256 | cut -d' ' -f1)
...
if [[ -z "$SHA256" ]]; then
    ...
    echo -e "${CYAN}   curl -sL https://github.com/kumamaki/claude-code-personalities/archive/$TAG.tar.gz | shasum -a 256${NC}"
else
...
echo "  brew install kumamaki/claude-code-personalities/claude-code-personalities"
echo
echo -e "${CYAN}Or with tap:${NC}"
echo "  brew tap kumamaki/claude-code-personalities"
echo "  brew install claude-code-personalities"
echo
echo -e "${CYAN}View release at:${NC}"
echo "  https://github.com/kumamaki/claude-code-personalities/releases/tag/$TAG"