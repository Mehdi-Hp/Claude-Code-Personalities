...
const GITHUB_REPO: &str = "kumamaki/claude-code-personalities";
...
impl VersionManager {
    ...
    pub fn new() -> Result<Self> {
        ...
        let cache_path = cache_dir.join("claude-code-personalities").join("version_cache.json");
    ...
    pub async fn download_asset(&self, asset: &GitHubAsset, target_path: &std::path::Path) -> Result<()> {
        use tokio::fs::File;
    ...
    async fn fetch_latest_release(&self) -> Result<GitHubRelease> {
        ...
        let response = client
            .get(&url)
            .header("User-Agent", format!("claude-code-personalities/{CURRENT_VERSION}"))
    ...
    async fn load_cached_version(&self) -> Result<VersionCache> {
        let content = tokio::fs::read_to_string(&self.cache_path).await
            .with_context(|| format!("Failed to read version cache: {}", self.cache_path.display()))?;
        
        serde_json::from_str(&content)
            .with_context(|| "Failed to parse version cache JSON")
    }
    ...
    async fn cache_version_info(&self, release: &GitHubRelease) -> Result<()> {
        ...
        tokio::fs::write(&self.cache_path, content).await
            .with_context(|| format!("Failed to write version cache: {}", self.cache_path.display()))?;
...
mod tests {
    ...
    fn test_platform_asset_matching() {
        ...
        assert_eq!(asset.unwrap().name, "claude-code-personalities-x86_64-apple-darwin");
    ...
    async fn test_version_manager_creation() {
        let vm = VersionManager::new().unwrap();
        assert!(vm.cache_path.to_string_lossy().contains("claude-code-personalities"));
    }
}