...
use crate::version::{format_changelog, format_version_comparison, VersionManager, CURRENT_VERSION};
...
pub async fn update_personalities(options: UpdateOptions) -> Result<()> {
    println!("{}", "Checking for Claude Code Personalities updates...".bold().blue());
    println!();
...
async fn check_and_get_release(
    ...
    if let Some(release) = update_info {
        if release.prerelease && !options.include_prereleases {
            print_info("Latest release is a pre-release. Use --include-prereleases to update to it.");
        ...
        if options.force {
            ...
            version_manager.get_latest_release().await
...
async fn perform_update(
    ...
    let platform = Platform::detect()
        .with_context(|| "Failed to detect current platform")?;
    ...
    let asset = version_manager.find_platform_asset(latest_release, &platform.target)
        .ok_or_else(|| anyhow!("No binary available for platform: {}", platform.description()))?;
...
fn setup_update_paths() -> Result<UpdatePaths> {
    let claude_dir = get_claude_dir()
        .with_context(|| "Failed to determine Claude directory")?;
    let current_binary = claude_dir.join("claude-code-personalities");
    let backup_binary = claude_dir.join(format!("claude-code-personalities.backup.{CURRENT_VERSION}"));
...
async fn download_and_verify_binary(
    ...
    verify_binary(&temp_binary.to_path_buf()).await
...
async fn backup_and_replace_binary(paths: &UpdatePaths) -> Result<()> {
    ...
    fs::copy(&paths.current_binary, &paths.backup_binary).await
        .with_context(|| format!("Failed to create backup: {}", paths.backup_binary.display()))?;
    print_success(&format!("Backup created: {}", paths.backup_binary.display()));
    
    print_info("Installing updated binary...");
    fs::rename(&paths.temp_binary, &paths.current_binary).await
    ...
    #[cfg(unix)]
    {
        ...
        let mut permissions = fs::metadata(&paths.current_binary).await
            .with_context(|| "Failed to get metadata for updated binary")?
            .permissions();
        permissions.set_mode(0o755);
        fs::set_permissions(&paths.current_binary, permissions).await
...
async fn verify_installation_and_cleanup(
    paths: &UpdatePaths,
    latest_version: &str,
) -> Result<()> {
    print_info("Verifying installation...");
    let new_version = get_binary_version(&paths.current_binary).await
    ...
    cleanup_old_backups(&paths.claude_dir).await
...
async fn verify_binary(binary_path: &PathBuf) -> Result<()> {
    ...
    #[cfg(unix)]
    {
        let content = fs::read(binary_path).await
        ...
        if content.len() >= 4 {
            ...
            if !is_elf && !is_macho && !is_fat_macho {
                return Err(anyhow!("Downloaded file does not appear to be a valid executable"));
            }
        }
    }
...
async fn cleanup_old_backups(claude_dir: &PathBuf) -> Result<()> {
    ...
    for (path, _) in backup_files.iter().skip(3) {
        if let Err(e) = fs::remove_file(path).await {
            print_warning(&format!("Failed to remove old backup {}: {}", path.display(), e));
        }
    }
...
fn print_update_success(old_version: &str, new_version: &str, binary_path: &Path) {
    println!("{}", "╔═══════════════════════════════════════════════════════════╗".bold().green());
    println!("{}", "║                                                           ║".bold().green());
    println!("{} {} {}", "║".bold().green(), "🎉 Claude Code Personalities Updated Successfully! 🎉".bold().white(), "║".bold().green());
    println!("{}", "║                                                           ║".bold().green());
    println!("{}", "╚═══════════════════════════════════════════════════════════╝".bold().green());
    println!();
    ...
    println!("  3. Check status with: {}", "claude-code-personalities status".cyan());
    println!();
    
    println!("{}", "Need Help?".bold().magenta());
    println!("  {} View configuration options", "claude-code-personalities config".cyan());
    println!("  {} Get help and usage info", "claude-code-personalities help".cyan());
    println!("  {} Report issues on GitHub", "https://github.com/Mehdi-Hp/claude-code-personalities/issues".cyan());
    println!();
    
    println!("{} Your Claude Code personalities are now up to date!", "🚀".yellow());
}
...
mod tests {
    ...
    async fn test_verify_binary_invalid_format() {
        ...
        temp_file.write_all(b"This is not an executable file").unwrap();
        temp_file.write_all(&vec![0; 2000]).unwrap(); // Pad to reasonable size
        temp_file.flush().unwrap();
    ...
    async fn test_cleanup_old_backups() {
        ...
        // Run cleanup
        cleanup_old_backups(&temp_dir.path().to_path_buf()).await.unwrap();
