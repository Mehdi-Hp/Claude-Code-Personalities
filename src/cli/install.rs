...
pub async fn install_personalities(options: InstallationOptions) -> Result<()> {
    ...
    if claude_dir.exists() {
        ...
        if options.interactive {
            ...
            if !create_dir {
                return Err(anyhow!("Installation cancelled: Claude directory is required"));
            }
        }
    ...
    // Step 3: Determine target binary location
    let target_binary = claude_dir.join("claude-code-personalities");
    
    // Step 4: Check if already installed
    if target_binary.exists() && !options.force_reinstall {
        if options.interactive {
            let reinstall = Confirm::new("Claude Code Personalities is already installed. Reinstall?")
                .with_default(false)
                .prompt()
    ...
    // Step 5: Copy binary to Claude directory
    print_info("Installing binary...");
    copy_binary(&current_binary, &target_binary).await
        .with_context(|| "Failed to install binary to Claude directory")?;
    print_success(&format!("Binary installed to: {}", target_binary.display()));
    
    // Step 6: Load Claude settings
    print_info("Loading Claude settings...");
    let mut settings = ClaudeSettings::load().await
        .with_context(|| "Failed to load Claude settings")?;
    
    // Step 7: Create backup if requested and settings exist
    if options.backup_existing && settings.settings_path.exists() {
        print_info("Creating settings backup...");
        let backup_path = settings.create_backup().await
            .with_context(|| "Failed to create settings backup")?;
        print_success(&format!("Backup created: {}", backup_path.display()));
    }
    
    // Step 8: Check current configuration
    ...
    // Step 9: Configure statusline
    print_info("Configuring statusline...");
    settings.configure_statusline(&target_binary)
    ...
    settings.configure_hooks(&target_binary)
    ...
    print_success(&format!("Settings saved to: {}", settings.settings_path.display()));
    ...
    if final_summary.is_fully_configured() {
        ...
        return Err(anyhow!("Installation verification failed - some components may not be configured correctly"));
    }
...
async fn copy_binary(source: &PathBuf, target: &PathBuf) -> Result<()> {
    ...
    fs::copy(source, target).await
        .with_context(|| format!("Failed to copy binary from {} to {}", source.display(), target.display()))?;
    
    // Make sure it's executable
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        let mut permissions = fs::metadata(target).await
            .with_context(|| format!("Failed to get metadata for {}", target.display()))?
            .permissions();
        permissions.set_mode(0o755); // rwxr-xr-x
        fs::set_permissions(target, permissions).await
            .with_context(|| format!("Failed to set executable permissions on {}", target.display()))?;
    }
...
fn get_current_binary_path() -> Result<PathBuf> {
    std::env::current_exe()
        .with_context(|| "Failed to determine current binary path")
}
...
fn print_installation_success(binary_path: &Path, settings_path: &Path) {
    println!("{}", "╔══════════════════════════════════════════════════════════╗".bold().green());
    println!("{}", "║                                                          ║".bold().green());
    println!("{} {} {}", "║".bold().green(), "🎉 Claude Code Personalities Installed Successfully! 🎉".bold().white(), "║".bold().green());
    println!("{}", "║                                                          ║".bold().green());
    println!("{}", "╚══════════════════════════════════════════════════════════╝".bold().green());
    println!();
    ...
    println!("  3. Customize settings with: {}", "claude-code-personalities config".cyan());
    println!();
    
    println!("{}", "What's Configured:".bold().yellow());
    println!("  {} Dynamic statusline with personality faces", ICON_CHECK.green());
    ...
    println!("  {} Check installation status", "claude-code-personalities status".cyan());
    println!("  {} Customize appearance", "claude-code-personalities config".cyan());
    println!("  {} Check for updates", "claude-code-personalities check-update".cyan());
    println!("  {} View all commands", "claude-code-personalities help".cyan());
    println!();
    
    println!("{} Start a new Claude Code session to see your personalities in action!", "🚀".yellow());
}
...
mod tests {
    ...
    async fn test_copy_binary() {
        ...
        source_file.write_all(b"#!/bin/bash\necho 'test binary'").unwrap();
