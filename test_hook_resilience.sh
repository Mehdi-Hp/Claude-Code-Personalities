...
echo "🧪 Testing hook resilience with concurrent execution..."
...
wait
echo "✅ Test 1 passed: No failures with parallel hooks"
echo ""
...
if [ $? -eq 0 ]; then
    echo "✅ Test 2 passed: New session handled gracefully"
else
    echo "❌ Test 2 failed: New session caused error"
    exit 1
fi
echo ""
...
wait
echo "✅ Test 3 passed: Mixed hook types handled"
echo ""
...
echo "✅ Test 4 passed: Rapid session lifecycle handled"
echo ""
...
echo "{\"session_id\":\"$CORRUPT_SESSION\",\"tool_name\":\"Read\",\"tool_input\":{\"file_path\":\"test.rs\"}}" | \
    "$BINARY" --hook post-tool
if [ $? -eq 0 ]; then
    echo "✅ Test 5 passed: Corrupted state file recovered gracefully"
else
    echo "❌ Test 5 failed: Corrupted state file caused error"
    exit 1
fi
rm -f "$STATE_FILE"
echo ""
...
echo ""
echo "✨ All tests passed! Hook system is resilient."
echo ""
